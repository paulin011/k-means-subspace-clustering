...
K-means leaves \kmresid{} of the variance unexplained and finds a different
...
is than the best, in percent of the best.}
\label{tab:methods}
\begin{tabular}{lrrr}
\hline
...
pairwise at NMI 0.68 to 0.72 (chance level 0.13). The k-means partitions are
less reproducible. Their three seeds agree only at NMI 0.57 to 0.58 despite
a tighter objective spread. Fitting at $K{=}256$ and
...
a factor of 15.7, so each cell is standardised against its own history. The top 0.1\% of standardised
...
2018 sudden-stratospheric-warming Arctic heat anomaly ($z{=}22.4$), the
December 2015 above-freezing North Pole event ($z{=}19.5$), the August 2016
Arctic cyclone, and the March 2022 East-Antarctic heatwave. The single
...
moves in latent space. Raw latent residual is not calibrated to physical
anomaly size. Only the standardised score recovers such events.
...
look global. The most extreme case appears somewhere in 36\% of all cells yet
holds half its tokens in only 4.4\% of the globe. Across the run 77 of 128
...
sea ice, with occupancy 1.0 in its core cells. Its residual is
...
mix is an almost pure seasonal harmonic ($R^2{=}0.992$). Interannual events are visible on top of the seasonal
cycle. In the Ni\~no 3.4 box, three of the
four El Ni\~no winters of the period (2015/16, 2018/19, 2019/20, all but the
weak 2014/15 event) have a mean anomaly score of 0.55 to 0.60 while four of
the five remaining winters sit at 0.12 to 0.18. The exception is the 2021/22
...
(SSW 2018, the above-freezing North Pole 2015, Antarctic heatwave 2022) are
recoverable
from the latents as spatially and temporally coherent outlier events after
per-cell standardisation, since latent residual size is not calibrated to
physical anomaly size. Spatially the latents are organised into regimes with
real territories, visible only under a mass threshold because almost every
cluster appears somewhere on the globe. Temporally the seasonal cycle dominates and is carried by a
