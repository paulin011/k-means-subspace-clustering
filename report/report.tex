...
all tokens (sizes \kcminmax) and the objective lands above the 100\% line,
because k-center can only place a centre on a token, never compute one.
Only its minimax radius is stable across seeds (146 to 148).
...
the total token variance of 5998, the share left unexplained, 100\% being
no clustering. \textbf{spread} is how much worse the worst of the 3 seeds
is than the best.}
\label{tab:methods}
\begin{tabular}{lrrr}
\hline
...
cluster never visits, and each title gives the smallest set of cells holding
half that cluster's tokens. Each panel has its own colour scale because
