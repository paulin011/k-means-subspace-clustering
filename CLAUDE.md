...
- `analyze_clusters.py` — algorithm-agnostic Markdown report generator; reads any `model.pt`/`assignments.pt` following the `cluster_io.py` schema (convergence, variance decomposition, per-cluster spatial/temporal stats, subspace affinity when `d>0`, optional held-out section from `holdout.json`, token-sample fingerprint + reproduce command) plus `dominant_cluster_map.png` (NESTED world map, cells colored by spectral seriation of the subspace-affinity matrix, or centroid-cosine affinity at `d=0`, so similar clusters share hues; the affinity matrix is built once in `build_affinity_matrix` and reused by both the coloring and the affinity table). The pure-numpy `healpix_nest2ring` / `healpix_ring_lonlat` helpers are validated (data-free): `nest2ring` is a bijection of `[0,12288)`; lon/lat are clean (lat ∈ [−88.5°, 88.5°], no NaN/inf); and the equal-area distribution matches analytics (50.0% of cells at |lat|<30°; 13.7% at |lat|>60° vs 13.4% theoretical). Backfills `sample.json` for pre-manifest runs. `python3 analyze_clusters.py --dir <out_dir> --out <report.md>`. **Every report metric is defined/interpreted in `METRICS.md`** — keep it in sync when changing the report. (Note: the committed copy as of 79b0f52 had a syntax error in the d80/EVR `add(...)` call from a botched "remove stale text" edit — fixed.)
